...
const BenefitsSection = () => {
  return (
    <section id="benefits" className="py-32 md:py-40 relative overflow-hidden">
          ...
          {benefits.map((b, i) => (
            <Reveal key={b.title} delay={0.08 * i} scale>
              ...
              >
                ...
                <div className="text-3xl font-mono font-bold text-warning mb-6 tracking-tight relative z-10">
                  {b.metric}
                </div>

                {/* Premium icon container with glow and animation */}
                <motion.div
                  animate={floatingAnimation(i * 0.2)}
                  className="relative mb-6"
                >
                  {/* Icon glow background */}
                  <div className="absolute inset-0 bg-primary/20 rounded-2xl blur-lg opacity-0 group-hover:opacity-100 transition-opacity duration-500 -z-10" />
                  
                  <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-primary/15 to-primary/5 border border-primary/20 flex items-center justify-center mx-auto group-hover:from-primary/25 group-hover:to-primary/10 group-hover:border-primary/40 transition-all duration-500 relative z-10">
                    <motion.div
                      animate={{ rotate: [0, 5, 0] }}
                      transition={{ duration: 4, delay: i * 0.3, repeat: Infinity }}
                    >
                      <b.icon className="h-6 w-6 text-primary/80 group-hover:text-primary/100 transition-colors duration-500" />
                    </motion.div>
                  </div>
                </motion.div>

                <h3 className="text-base font-display font-semibold text-foreground mb-2 tracking-tight relative z-10">
                  {b.title}
                </h3>
                <p className="text-silver text-xs leading-relaxed relative z-10">{b.desc}</p>
              </motion.div>
            </Reveal>
          ))}
        </div>
